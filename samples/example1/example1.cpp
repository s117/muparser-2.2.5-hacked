...
value_type Bor(value_type v1, value_type v2) { return (unsigned)v1 | (unsigned)v2; }
...
int IsBinValue(const char_type *a_szExpr, int *a_iPos, value_type *a_fVal) {
	if (a_szExpr[1] == 0 || (a_szExpr[0] != '0' || a_szExpr[1] != 'b'))
		return 0;
	const char_type *endptr;
	unsigned long iVal(0);

	iVal = wcstoul(a_szExpr + 2, (char_type **)&endptr, 2);

	if (endptr - a_szExpr == 2) {
		*a_iPos += 1;
		*a_fVal = iVal;
		return 1;
	} else {
		*a_iPos += endptr - a_szExpr;
		*a_fVal = iVal;
		return 1;
	}
}
...
extern void Initialize(mu::Parser *parser);
static double before_1;
static double after_1;
static value_type* PreQueryExtVariableCb(string_type& symbol) {
	if (symbol == _T("before"))
		return &before_1;
	else
	return nullptr;
}
static value_type* PostQueryExtVariableCb(string_type& symbol) {
	if (symbol == _T("after"))
		return &after_1;
	else
		return nullptr;
...
void Calc() {
	...
	//parser.SetThousandsSep('');
#endif
	parser.SetPreQueryExtVariableCb(PreQueryExtVariableCb);
	parser.SetPostQueryExtVariableCb(PostQueryExtVariableCb);
	...
	parser.DefineOprt(_T("mul"), Mul, 15);
	*/
...
int main(int, char**) {
	...
	int i = IsHexValue(_T("0x0"), &pos, &rtn);*/
